  ...
  return (
    <form onSubmit={handleSave} className="space-y-4">
      ...
      <div className="space-y-2">
        <p className="text-xs text-zinc-500 font-medium uppercase tracking-wide">Quick examples</p>
        <div className="flex flex-wrap gap-2">
          {EXAMPLES.map((ex) => (
            <button
              ...
              className="text-xs px-3 py-1.5 glass rounded-full text-zinc-400 hover:text-brand-300 hover:border-brand-500/40 transition-all border border-white/10"
            >
      ...
      <div className="flex items-center gap-3">
        ...
        {isDirty && !saving && (
          <span className="text-xs text-zinc-500">You have unsaved changes</span>
        )}
      </div>
    </form>
  );
